#include <tripi/tripiDGP.h>

void TripiDGP::initDataEIP158(){
...
bool TripiDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule TripiDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t TripiDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t TripiDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t TripiDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t TripiDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool TripiDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void TripiDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void TripiDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void TripiDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void TripiDGP::createParamsInstance(){
...
dev::Address TripiDGP::getAddressForBlock(unsigned int blockHeight){
...
void TripiDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TripiDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TripiDGP::parseStorageOneUint64(uint64_t& value){
...
void TripiDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule TripiDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;

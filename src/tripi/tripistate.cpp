...
TripiState::TripiState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = TripiState::openDB(_path + "/tripiDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

TripiState::TripiState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute TripiState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, TripiTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createTripiAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            tripi::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> TripiState::vins() const // temp
{
...
void TripiState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* TripiState::vin(dev::Address const& _a) const
{
    return const_cast<TripiState*>(this)->vin(_a);
}

Vin* TripiState::vin(dev::Address const& _addr)
{
...
// void TripiState::commit(CommitBehaviour _commitBehaviour)
// {
...
void TripiState::kill(dev::Address _addr)
{
...
void TripiState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void TripiState::deleteAccounts(std::set<dev::Address>& addrs){
...
void TripiState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void TripiState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;

...
#include <tripi/storageresults.h>


extern std::unique_ptr<TripiState> globalState;
...
using ExtractTripiTX = std::pair<std::vector<TripiTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // tripi
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashTripiRoot,
    const dev::h256& oldHashStateRoot, const std::vector<TripiTransaction>& transactions);
...
class TripiTxConverter{

public:

    TripiTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionTripiTransactions(ExtractTripiTX& tripiTx);
...
private:
    ...
    TripiTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<TripiTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}

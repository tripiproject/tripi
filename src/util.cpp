...
const char * const BITCOIN_CONF_FILENAME = "tripi.conf";
const char * const BITCOIN_PID_FILENAME = "tripid.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tripi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tripi
    // Mac: ~/Library/Application Support/Tripi
    // Unix: ~/.tripi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tripi";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tripi";
#else
    // Unix
    return pathRet / ".tripi";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Tripi Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Tripi Core developers";
    }

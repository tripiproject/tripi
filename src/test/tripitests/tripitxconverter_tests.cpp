...
void checkResult(bool isCreation, std::vector<TripiTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    TripiTxConverter converter(transaction, NULL);
    ExtractTripiTX tripiTx;
    BOOST_CHECK(converter.extractionTripiTransactions(tripiTx));
    std::vector<TripiTransaction> result = tripiTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(tripitxconverter_tests, TestingSetup)

...
inline void initState(){
    ...
    const std::string dirTripi = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<TripiState>(new TripiState(dev::u256(0), TripiState::openDB(dirTripi, hashDB, dev::WithExisting::Trust), dirTripi + "/tripiDB", dev::eth::BaseState::Empty));
...
inline dev::Address createTripiAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline TripiTransaction createTripiTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    TripiTransaction txEth;
    if(recipient == dev::Address()){
        txEth = TripiTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = TripiTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<TripiTransaction> txs){

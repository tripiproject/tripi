...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given tripi address.\n"
            "DEPRECATION WARNING: Parts of this command have been deprecated and moved to getaddressinfo. Clients must\n"
            "transition to using getaddressinfo to access this information before upgrading to v0.18. The following deprecated\n"
            "fields have moved to getaddressinfo and will only be shown here with -deprecatedrpc=validateaddress: ismine, iswatchonly,\n"
            "script, hex, pubkeys, sigsrequired, pubkey, addresses, embedded, iscompressed, account, timestamp, hdkeypath, kdmasterkeyid.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The tripi address to validate\n"
            "\nResult:\n"
            "{\n"
...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.pushKV("maxblocksize", (uint64_t)tripiDGP.getBlockSize(chainActive.Height()));
    obj.pushKV("mingasprice", (uint64_t)tripiDGP.getMinGasPrice(chainActive.Height()));
    obj.pushKV("blockgaslimit", (uint64_t)tripiDGP.getBlockGasLimit(chainActive.Height()));
...
static UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "createmultisig nrequired [\"key\",...] ( \"address_type\" )\n"
            ...
            "2. \"keys\"                       (string, required) A json array of keys which are tripi addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"                    (string) tripi address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tripi address to use for the signature.\n"
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "tripi-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\ntripi-cli has the option -getinfo to collect and format these in the old format."
    );
}

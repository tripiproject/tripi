...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // tripi
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // tripi
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // tripi
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // tripi
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination tripiSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(tripiSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&tripiSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of tripi addresses\n"

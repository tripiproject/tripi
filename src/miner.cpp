...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setTripiSchedule(tripiDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = tripiDGP.getBlockSize(nHeight);
    minGasPrice = tripiDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = tripiDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    TripiTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractTripiTX resultConverter;
    if(!convert.extractionTripiTransactions(resultConverter)){
    ...
    for(TripiTransaction tripiTransaction : tripiTransactions){
        txGas += tripiTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + tripiTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(tripiTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, tripiTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "tripistake";
...
void StakeTripis(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{

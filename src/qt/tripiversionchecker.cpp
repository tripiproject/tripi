...
#define paternVersion "tripi-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

TripiVersionChecker::TripiVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

TripiVersionChecker::~TripiVersionChecker()
{

}

bool TripiVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> TripiVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(TRIPI_RELEASES)));
    QEventLoop event;
...
Version TripiVersionChecker::getMaxReleaseVersion()
{

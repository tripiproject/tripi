...
*Setup instructions for a Gitian build of Tripi Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tripi
...
- [Building Tripi Core](#building-tripi-core)
...
Clone the git repositories for tripi and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tripiproject/tripi --recursive
git clone https://github.com/tripiproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/tripi/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the tripi repository under 'Fetch and create inputs' to install sources which require
...
To build Tripi Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tripi repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tripi/.git/
...
For example:
```bash
URL=https://github.com/laanwj/tripi.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tripi=${COMMIT} --url tripi=${URL} ../tripi/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tripi=${COMMIT} --url tripi=${URL} ../tripi/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tripi=${COMMIT} --url tripi=${URL} ../tripi/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-tripi:
./bin/gbuild --commit tripi=${COMMIT},cpp-eth-tripi=${ETHCOMMIT} --url tripi=${URL},cpp-eth-tripi=${ETHURL} ../tripi/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the tripi git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tripi/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/tripiproject/tripi-detached-sigs.git

BTCPATH=/some/root/path/tripi
SIGPATH=/some/root/path/tripi-detached-sigs

./bin/gbuild --url tripi=${BTCPATH},signature=${SIGPATH} ../tripi/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tripi-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tripi-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tripi-osx-build.assert
```
...
[tripiproject/gitian.sigs](https://github.com/tripiproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@tripi.org) and he will commit them.